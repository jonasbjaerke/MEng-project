...
from ..utils import parse_dt, get_json
...
class UserFeatureExtractor:
    ...
    def get_last_n_posts_features(self, history, exclude_post_id, n=50):
        """
        Collect and aggregate text/message features from the last n
        historical posts, excluding the current post P.
        """
        if not history:
            return {}
        ...
        post_ids = post_ids[-n:]  # last n posts in history order
    ...
    def history_stats(self, history, exclude_post_id):
        ...
        reposts = sum(
            1 for h in history
            if (
                h.get("activity_type") == "repost"
                or h.get("activity_type") == "reply"
            )
            and h.get("post_uri") != exclude_post_id
            and h.get("parent_post_uri") != exclude_post_id
        )
        ...
        for h in history:
            if h.get("activity_type") in {"post", "reply"}:
                total_likes += h.get("like_count", 0)
                total_reposts += h.get("repost_count", 0)
                total_replies += h.get("reply_count", 0)
                total_quotes += h.get("quote_count", 0)
    ...
    def reposts_from_author(history, author_did, exclude_post_id):
        return sum(
            ...
            and (
                h.get("post_author_did") == author_did
                or h.get("parent_author_did") == author_did
            )
            and h.get("post_uri") != exclude_post_id
            and h.get("parent_post_uri") != exclude_post_id
        )
    ...
    def calc_features(self, A_id, S_id, P_id, post, label):
        ...
        row = {
            ...
            "U-HA_S_MentionR": S_m_A,
            "U-HA_S_MentionPerR": S_m_A_per,
            "U-HA-R_repostsS": self.reposts_from_author(A["history"], S_id, P_id),
            "U-P_SR_followersDiff": S["stats"]["followers"] - A["stats"]["followers"],
            "U-P_R_activeBeforeP": (
                int(parse_dt(A["profile"].get("created_at")) < P_time)
                if A["profile"].get("created_at") else 0
            ),
        ...
        for prefix, user in [("R", A), ("S", S)]:
            ...
            row.update({
                ...
                f"U-P_{prefix}_ProfileUrl": int(
                    "http" in (user["profile"]["description"] or "")
                ),
            })
        ...
        for k, v in A_hist_msg_features.items():
            row[f"M-H_R_{k}"] = v

        # Sender history (S)
        S_hist_msg_features = self.get_last_n_posts_features(
            S["history"],
            P_id,
            n=50,
        )
        for k, v in S_hist_msg_features.items():
            row[f"M-H_S_{k}"] = v
